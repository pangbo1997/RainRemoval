...
def prepare_data_RainTrainL(data_path, patch_size, stride):
    ...
    save_target_path = os.path.join(data_path, 'train_target2.h5')
    save_input_path = os.path.join(data_path, 'train_input2.h5')
...
class Rain100lDataset(BaseDataset):
    def __init__(self, opt):
        BaseDataset.__init__(self, opt)

        self.data_path = opt.dataroot

        if not os.path.exists(os.path.join(self.data_path,'train_target2.h5')):
            prepare_data_RainTrainL(opt.dataroot,120,40)

        target_path = os.path.join(self.data_path, 'train_target2.h5')
        input_path = os.path.join(self.data_path, 'train_input2.h5')
